...
#[allow(dead_code)]
#[derive(Debug)]
pub enum SheepError<'a> {
    PermissionError(PermissionType<'a>),
    MissingError(&'a str),
    UnsuccessfulError(&'a str),
}

impl fmt::Display for SheepError<'_> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match *self {
            SheepError::PermissionError(perm) =>
                write!(f, "{}", perm),
                SheepError::MissingError(missing) => write!(f, "Please provide a {}!", missing),
                SheepError::UnsuccessfulError(cmd) =>
